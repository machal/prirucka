...
<div class="rwd-scrollable fs-6" markdown="1">
...
Hodnoty vnitřního zobrazení zapínají uvnitř dotčeného prvku nový kontext formátování ([formatting context](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Flow_Layout/Intro_to_formatting_contexts)). No a ten mívá vliv na způsob zobrazení vnitřních prvků, případně na použití dalších vlastností na nich. 
...
- `flow`  
Pokud vím, toto zatím není v prohlížečích podporováno. Hodnota `flow` zapíná formátování tokem (flow layout), což je běžný způsob zobrazení, který automaticky zapnou hodnoty `block`, `inline` a `inline-block`.
- `flow-root`  
Vytvoří kontejneru blokového kontextu (jako `display:block`) a rozloží jeho obsah pomocí rozložení tokem (flow layout). Vždy vytvoří nový kontext formátování bloku pro svůj obsah, takže není například nutné mazat „floaty“ pomocí [clearfixu](https://learnlayout.com/clearfix.html). Na této hodnotě je zajímavé, že ji podporují všechny moderní prohlížeče. Internet Explore nikoliv. [caniuse.com/flow-root](https://caniuse.com/flow-root)
- `ruby`  
Tohle je exotické a pro středoevropské prostředí nepotřebné. „Ruby anotace“ jsou krátké řady textu umístěné vedle základního textu, které se primárně používají ve východoasijské typografii jako vodítko pro výslovnost.
...
`display:flow` v prohlížečích nefunguje, ale jako by fungoval… „Tokové“ to totiž výchozí zobrazení.

Zajímavější bude ten druhý CodePen. Všechny tři vnitřní prvky jsou „tekoucí“, floatové:
...
Modely zobrazení, které vynucují vnitří rozvržení, jako je `display:table` a `display:ruby`, mají složitou vnitřní strukturu s několika různými rolemi, které mohou jejich  potomci plnit.

Jak je uvedeno výše, zápis `display:table` sice vytvoří kontejner tabulky, ale ten vytvoří kontext formátování bloku. Nedosáhneme tím tedy tabulkového zobrazení. K tomu bychom potřebovali další prvky, které reprezentují řádky a buňky tabulky se správnými hodnotami vlastnosti `display` (`table-row`, `table-cell`…). Je to prostě složitější, takže zájemce lačnící po tomhle způsobu zobrazení [pošlu jinam](https://colintoh.com/blog/display-table-anti-hero).
...
Ke všem možným metodám ovlivnění vykreslování boxíků na obrazovku musíme přidat i metody nevykreslování. K tomu slouží tyto dvě hodnoty.
...
Podpora nezobrazení pomocí `none` je samozřejmě plná. Hodnotu `contents` zvládají všechny prohlížeče kromě Internet Exploreru, ale podle webu CanIUse je s ní spojeno několik chyb v prohlížečích vycházejících z jádra Chromium. [caniuse.com/css-display-contents](https://caniuse.com/css-display-contents)
...
Vzpomínáte si, když jsem psal o různých typech zobrazení – vnitřním a vnějším? Pokud ne, rychle proskenujte začátek tohoto textu nebo jeho nadpisy. Pak pochopíte, proč mě víceslovné hodnoty pro vlastnost `display` dává smysl.
